...
public class SearchService {
    ...
    public Goods buildGoods(Spu spu) {
        ...
        //查询分类
        List<Category> categories = categoryClient.CategoryByIds(Arrays.asList(spu.getCid1(), spu.getCid2(), spu.getCid3()));
        ...
        //获得所有sku价格
//        List<Long> priceList = skus.stream().map(sku -> sku.getPrice()).collect(Collectors.toList());
        List<Long> priceList = new ArrayList<>();
        //对skus进行处理，不用多余字段
        List<Map<String, Object>> skuList = new ArrayList<Map<String, Object>>();
        for (Sku sku : skus) {
            priceList.add(sku.getPrice());//处理价格
        ...
        goods.setSkus(JsonUtils.serialize(skuList));//所有sku的json格式
...
/*AggregatedPage<Goods> pageInfo = (AggregatedPage<Goods>)goodsRepository.search(queryBuilder.build());
        ...
        if (categories != null && categories.size() == 1) {
            spec = biuldSpecificationAgg(categories.get(0).getId(), basicQuery);//在原来的基础上继续聚合
        }

        return new SearchResult(total, (long) totalPage, goodsList, categories, brands, spec);
    }
    ...
    private List<Map<String, Object>> biuldSpecificationAgg(Long cid, QueryBuilder basicQuery) {
        ...
        for (SpecParam param : params) {
            String name = param.getName();
            StringTerms tems = aggs.get(name);
            List<String> options = tems.getBuckets().stream().map(bucket -> bucket.getKeyAsString()).collect(Collectors.toList());
            //准备map
            Map<String,Object> map = new HashMap<>();
            map.put("k",name);
    ...
    private List<Brand> parseBrandAgg(LongTerms terms) {
        try {
            ...
            List<Brand> brands = brandClient.queryBrandByIdsForAgg(ids);

            return brands;
        } catch (Exception e) {
            log.error("[搜索服务] 品牌查询异常");
            return null;
        }
    }
    ...
    private List<Category> parseCategoryAgg(LongTerms terms) {
        try {
            List<Long> ids = terms.getBuckets().stream().map(bucket -> bucket.getKeyAsNumber().longValue()).collect(Collectors.toList());
            List<Category> categories = categoryClient.CategoryByIds(ids);
            return categories;
        } catch (Exception e) {
            log.error("[搜索服务] 商品分类查询异常");
            return null;
        }
    }
