...
public class PayHelper {
    ...
    public String createOrder(Long orderId, Long totalPay, String desc){
        try {
            ...
            log.error("[订单服务] 微信下单创建预交易订单异常失败", e);
            return null;
        }
    }
...
//        if("FAIL".equals(returnCode)){
            // 通信失败
            log.error("[订单服务] 微信下单通信失败,失败原因:{}", result.get("return_msg"));
            throw new LyException(ExceptionEnums.WX_PAY_ORDER_FAIL);
        }

        // 判断业务标示
        String resultCode = result.get("result_code");
//        if("FAIL".equals(resultCode)){使用常量
        if(FAIL.equals(resultCode)){
            // 通信失败
            log.error("[订单服务] 微信下单业务失败,错误码:{}, 错误原因:{}",
...
//            String sign1 = WXPayUtil.generateSignature(result, payConfig.getKey(), WXPayConstants.SignType.HMACSHA256);
            String sign1 = WXPayUtil.generateSignature(result, payConfig.getKey(), SignType.HMACSHA256);//不确定返回签名是哪种就个生成一个进行比对
//            String sign2 = WXPayUtil.generateSignature(result, payConfig.getKey(), WXPayConstants.SignType.MD5);
            ...
            log.error("[订单服务] 微信支付校验签名失败，数据：{}", result);
            throw new LyException(ExceptionEnums.WX_PAY_ORDER_FAIL);
        }
    }
